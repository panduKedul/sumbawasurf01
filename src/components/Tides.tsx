        ...
        <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-4 md:p-6">
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              ...
              <p className="text-base md:text-lg text-gray-200 mt-2 drop-shadow">
                {selectedSpot.name}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-4">
            <select
              ...
              className="input-elegant px-4 py-2 rounded-lg w-full sm:max-w-md backdrop-blur-sm"
            >
              {spots.map((spot) => (
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            <div>
              <h3 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6 text-center flex items-center justify-center">
                <Activity className="w-5 md:w-6 h-5 md:h-6 text-neon-blue mr-2" />
                Today's Tides
              </h3>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                {todayTides.map((tide, index) => (
                  <div
                    key={index}
                    className={`card-elegant p-4 md:p-6 transition-all duration-300 hover:scale-105 ${
                      tide.type === 'high'
            ...
            <div className="card-elegant p-4 md:p-8">
              <h3 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6 text-center flex items-center justify-center">
                <BarChart3 className="w-5 md:w-6 h-5 md:h-6 text-neon-blue mr-2" />
                24-Hour Tide Chart
              </h3>
              
              <div className="relative h-48 md:h-64 bg-dark-300 rounded-xl p-4 md:p-6 border border-dark-400">
                <div className="absolute inset-4 md:inset-6">
                  {/* Y-axis labels */}
                  <div className="absolute left-0 top-0 bottom-0 flex flex-col justify-between text-xs md:text-sm text-gray-400">
                    <span>2.0m</span>
                    <span>1.5m</span>
                    <span>1.0m</span>
                    <span>0.5m</span>
            ...
            <div className="card-elegant p-4 md:p-6">
              <h3 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6 text-center">7-Day Tide Overview</h3>
              <div className="overflow-x-auto">
                <div className="flex space-x-3 md:space-x-4 pb-4" style={{ minWidth: 'max-content' }}>
                  {Object.entries(weeklyTides).map(([date, tides], dayIndex) => (
                    <div key={dayIndex} className="flex-shrink-0 bg-dark-300 rounded-lg p-3 md:p-4 border border-dark-400 min-w-[160px] md:min-w-[200px]">
                      <h4 className="text-center font-bold text-white mb-3 md:mb-4">{formatDateShort(date)}</h4>
            ...
            <div className="card-elegant p-4 md:p-6">
              <h3 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6 text-center">Upcoming Tides</h3>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[600px]">
                  <thead>
                    <tr className="border-b border-dark-400">
                      <th className="text-left py-3 px-2 md:px-4 text-gray-300 font-semibold">Date & Time</th>
                      <th className="text-center py-3 px-2 md:px-4 text-gray-300 font-semibold">Type</th>
                      <th className="text-right py-3 px-2 md:px-4 text-gray-300 font-semibold">Height</th>
                      <th className="text-center py-3 px-2 md:px-4 text-gray-300 font-semibold">Phase</th>
                    </tr>
                  </thead>
                  <tbody>
                    {upcomingTides.map((tide, index) => (
                      <tr key={index} className="border-b border-dark-400/50 hover:bg-dark-300/50 transition-colors">
                        <td className="py-3 px-2 md:px-4">
                          <div>
                            <div className="font-semibold text-white text-sm md:text-base">{formatDate(tide.time)}</div>
                            <div className="text-xs md:text-sm text-gray-400">{formatTime(tide.time)}</div>
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-2 md:px-4 text-right">
                          <span className="text-lg md:text-xl font-bold text-white">{tide.height.toFixed(1)}m</span>
                        </td>
                        <td className="py-3 px-2 md:px-4 text-center">
                          {getPhaseIcon(tide.time)}
                        </td>
            ...
            <div className="card-elegant p-4 md:p-6 bg-gradient-to-br from-dark-300 to-dark-400">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-4 md:mb-6">
                <div className="text-center">
                  <div className="text-neon-blue font-semibold mb-2 text-sm md:text-base">Wave Type</div>
                  <div className="text-gray-300 text-sm md:text-base">{selectedSpot.waveType}</div>
                </div>
                <div className="text-center">
                  <div className="text-neon-blue font-semibold mb-2 text-sm md:text-base">Skill Level</div>
                  <div className="text-gray-300 text-sm md:text-base">{selectedSpot.skillLevel}</div>
                </div>
                <div className="text-center">
                  <div className="text-neon-blue font-semibold mb-2 text-sm md:text-base">Best Season</div>
                  <div className="text-gray-300 text-sm md:text-base">{selectedSpot.bestSeason}</div>
                </div>
                <div className="text-center">
                  <div className="text-neon-blue font-semibold mb-2 text-sm md:text-base">Tide Conditions</div>
                  <div className="text-gray-300 text-sm md:text-base">{selectedSpot.tideConditions}</div>
                </div>
              </div>
              <div className="text-center">
