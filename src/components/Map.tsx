...
const FlyToSelectedSpot: React.FC<{ coordinates: [number, number] }> = ({ coordinates }) => {
  const map = useMap();
  map.flyTo(coordinates, 12, { duration: 1.5 });
  return null;
};
...
export default function Map({ spots, onSelectSpot, selectedSpot }: MapProps) {
  ...
  const defaultCenter: [number, number] = [-8.85, 116.75];

  return (
    <div className="w-full h-full rounded-xl overflow-hidden border" style={{ height: '100%', minHeight: '300px' }}>
      <MapContainer
        ...
        {spots.map((spot) => (
          ...
          >
            <Popup>
              ...
              {spot.waveType} - {spot.skillLevel}
            </Popup>
          </Marker>
        ))}
        {selectedSpot && <FlyToSelectedSpot coordinates={selectedSpot.coordinates} />}
      </MapContainer>
    </div>
  );
}
