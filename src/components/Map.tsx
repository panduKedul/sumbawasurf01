...
export default function Map({ spots, onSelectSpot, selectedSpot }: MapProps) {
  ...
  const MapFallback = () => (
    <div className="w-full h-full bg-gradient-to-br from-dark-300 to-dark-400 rounded-xl border border-dark-400 flex flex-col overflow-hidden">
      <div className="p-3 sm:p-4 md:p-6 text-center flex-1 flex flex-col justify-center">
        <div className="mb-4 md:mb-6">
          <div className="w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br from-neon-blue to-primary-600 rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4">
            <Navigation className="w-6 h-6 md:w-8 md:h-8 text-white" />
          </div>
          <h3 className="text-lg md:text-2xl font-bold text-white mb-2">West Sumbawa Surf Spots</h3>
          <p className="text-sm md:text-base text-gray-400 mb-4 md:mb-6">Interactive map of {spots.length} surf locations</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 max-w-4xl mx-auto overflow-y-auto max-h-64 md:max-h-80">
          {spots.map((spot) => (
            <button
              ...
              className={`p-3 md:p-4 rounded-lg border transition-all duration-300 hover:scale-105 text-left ${
            ...
            >
              <div className="flex items-start space-x-2 md:space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-semibold text-white text-xs md:text-sm mb-1 truncate">{spot.name}</h4>
  ...
  return (
    <div className="relative w-full h-full">
      {/* Google Maps Embed */}
      {!mapError ? (
        <div className="w-full h-full rounded-xl overflow-hidden border border-dark-400">
          <iframe
            src={googleMapsUrl}
            width="100%"
            height="100%"
      ...
      {!mapError && (
        <>
          <div className="absolute top-2 md:top-4 left-2 md:left-4 bg-dark-200/95 backdrop-blur-sm rounded-lg p-2 md:p-3 shadow-lg border border-dark-400">
            <div className="flex items-center space-x-2">
              <MapPin className="w-3 h-3 md:w-4 md:h-4 text-neon-blue" />
              <span className="text-xs md:text-sm font-medium text-white">West Sumbawa</span>
            </div>
          ...
          {/* Selected Spot Info */}
          {selectedSpot && (
            <div className="absolute bottom-2 md:bottom-4 left-2 md:left-4 right-2 md:right-4 bg-dark-200/95 backdrop-blur-sm rounded-lg p-3 md:p-4 shadow-lg border border-dark-400">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-sm md:text-lg text-white mb-1 truncate">{selectedSpot.name}</h3>
                  <p className="text-xs md:text-sm text-gray-300 mb-2 line-clamp-2">{selectedSpot.description}</p>
                ...
                {selectedSpot.forecastUrl && (
                  <a
                    ...
                    className="flex items-center space-x-1 text-neon-blue hover:text-neon-blue/80 text-xs md:text-sm transition-colors ml-2 md:ml-4 flex-shrink-0"
                  >
